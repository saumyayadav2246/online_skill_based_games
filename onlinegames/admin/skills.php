...
$total_skills_query = $conn->query("SELECT COUNT(*) as total FROM skills");
$total_skills_count = $total_skills_query->fetch_assoc()['total'];

$skills_list = $conn->query("SELECT * FROM skills ORDER BY name ASC");
?>
...
<head>
    ...
    <title>Skills List - Brain League</title>
    ...
    <style>
        ...
        .sidebar-nav { background: linear-gradient(180deg, #0f172a 0%, #1e293b 100%); border-radius: 15px; }
        .sidebar-nav .nav-link { color: rgba(255,255,255,0.9); padding: 12px 20px; border-radius: 10px; margin: 4px 12px; transition: all 0.3s ease; }
        ...
        .stat-card { border: none; border-radius: 20px; height: 120px; background: linear-gradient(135deg, #059669 0%, #10b981 100%); }
        .table-container { border: none; border-radius: 15px; background: white; }
        .table thead th { background-color: #f8fafc; color: #64748b; font-size: 0.8rem; text-transform: uppercase; border-top: none; }
    </style>
</head>
<body>
    ...
    <div class="offcanvas offcanvas-start" tabindex="-1" id="sidebarMenu" style="width: 280px;">
        ...
        <div class="offcanvas-body p-3" style="background-color: #0f172a;">
            <ul class="nav flex-column nav-pills sidebar-nav h-100 shadow-none">
                ...
                <li class="nav-item"><a class="nav-link active" href="skills.php"><i class="bi bi-mortarboard-fill me-2"></i>Skills</a></li>
    ...
    <main class="container-fluid py-4 px-3 px-md-4">
        <div class="row g-4">
            ...
            <div class="col-lg-9 col-xl-10">
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="card stat-card text-white shadow">
                            <div class="card-body d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="text-uppercase small fw-bold">Total Skills</h6>
                                    <h2 class="fw-bold mb-0"><?php echo number_format($total_skills_count); ?></h2>
                                </div>
                ...
                <div class="card shadow-sm table-container">
                    <div class="card-header bg-white py-3 border-0">
                        <h5 class="fw-bold mb-0 text-dark">Skills</h5>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                <?php if($skills_list->num_rows > 0): ?>
                                    <?php while($skill = $skills_list->fetch_assoc()): ?>
                                    <tr>
                                        <td class="ps-4 text-muted small">#<?php echo $skill['id']; ?></td>
                                        <td><span class="fw-semibold text-dark"><?php echo htmlspecialchars($skill['name']); ?></span></td>
                                        <td class="pe-4 text-muted"><?php echo date('M d, Y', strtotime($skill['created_at'])); ?></td>
                                    </tr>
                                    <?php endwhile; ?>
                                <?php else: ?>
                                    <tr>
